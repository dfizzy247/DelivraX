...
function makeTable(result, l1, l2){
	...
	// Handle empty tables - need to fetch schema to create modal
	if (result.length === 0) {
		table += '<div style="width:auto;margin:18px;" class="card border-left-warning shadow"><div class="alert alert-info m-3">No records found. Click the "Add" button above to create your first record.</div></div>';
		// Create empty modal - will be populated by makeModalsForEmptyTable
		var modals = makeModalsForEmptyTable(tablename);
...
function makeModals(result){
	...
	modal += '</form></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button><button type="button" class="btn btn-primary" onclick="createRecord(\''+tablename+'\');">Create Record</button></div></div></div></div>';
...
function createRecord(table){
	...
	// Get form data and convert date formats
	$('form[name="createForm"] *').filter(':input').each(function () {
		if (this.value == ''){
			this.value='None';
		} else if (this.name && (this.name.toLowerCase().includes('date') || this.name === 'DOB')) {
			// Convert MM/DD/YYYY or DD/MM/YYYY to YYYY-MM-DD for MySQL
			var dateVal = this.value;
			// Check if it's in MM/DD/YYYY or DD/MM/YYYY format
			if (dateVal.includes('/')) {
				...
				if (parts.length === 3) {
					...
					console.log('Converted date:', dateVal, 'to', this.value);
				}
			}
		}
	});
	var str = $('form[name="createForm"]').serialize();
	console.log('Form data (serialized):', str);
	
	// Replace null values but DON'T replace all whitespace
	str = str.replace(/null/g,'None');
	// Only replace spaces in URL encoding, not the actual + signs
	str = str.replace(/%20/g, '+');
	
	console.log('Final URL:', "/api/" + table + "?" + str);
	
	$.ajax({
	  method: "POST",
	  url: "/api/" + table + "?" + str,
	  success: function(dat) {
	  		console.log('Record created successfully:', dat);
	  		getData(table,10,0);
	  		$('#addModal').modal('hide');
		},
	  error: function(xhr, status, error) {
	  		...
	  		alert('Error creating record: ' + (xhr.responseText || error));
		}
	});	
}
...
// Function to create modal for empty tables
function makeModalsForEmptyTable(tablename) {
	// Define table schemas with their fields
	var tableSchemas = {
		'drivers': ['DriverID', 'Username', 'Password', 'FirstName', 'LastName', 'DOB', 'NINo', 'DrivingLicenseNo', 'DrivingLicensePic', 'Email', 'Phone', 'Address1', 'Address2', 'City', 'PostCode', 'Country', 'Location', 'LicenseNumber', 'VehicleID', 'Status', 'LastConnected', 'DateCreated'],
		'vehicles': ['VehicleID', 'RegistrationNumber', 'Make', 'Model', 'Year', 'Color', 'Capacity', 'Status', 'DateAdded'],
		'customers': ['CustomerID', 'FirstName', 'LastName', 'CompanyName', 'Email', 'Phone', 'Address', 'City', 'PostalCode', 'DateCreated'],
		'jobs': ['JobID', 'CustomerID', 'DriverID', 'PickupLocation', 'DeliveryLocation', 'PickupDate', 'DeliveryDate', 'Status', 'Priority', 'Notes', 'Picture1', 'Picture2', 'DateCreated', 'DateDelivered'],
		'locations': ['LocationID', 'Name', 'Address', 'City', 'PostalCode', 'Latitude', 'Longitude', 'Type', 'DateCreated'],
		'receipts': ['ReceiptID', 'DriverID', 'Amount', 'Type', 'Picture', 'Description', 'DateCreated'],
		'parcels': ['ParcelID', 'JobID', 'TrackingNumber', 'Description', 'Weight', 'Dimensions', 'Status', 'DateCreated']
	};
	
	var headers = tableSchemas[tablename.toLowerCase()] || [];
	
	if (headers.length === 0) {
		return '<div class="alert alert-warning">Unable to create form for this table.</div>';
	}
	
	var modal = '<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog modal-xl" role="document"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Create Record</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><div class="modal-body"><form name="createForm" id="createForm">';
	
	var oddCheck = 0;
	for (var i = 1; i < headers.length; i++) { // Skip first field (ID)
		var header = headers[i];
		var disabled = '';
		
		if (header.toLowerCase().includes('datecreated') || header.toLowerCase().includes('lastconnected')) {
			disabled = 'disabled="disabled"';
		}
		
		if (oddCheck % 2 === 1) {
			if (header.toLowerCase().includes('date') || header.toLowerCase().includes('lastconnected')) {
				modal += '<div class="row"><div class="col-sm"><label>' + header + '</label></div><div class="col-sm"><div id="date" class="input-group"><input type="text" ' + disabled + ' class="form-control datepicker_recurring_start" name="' + header + '" /><div class="input-group-append"><span class="input-group-text" id="basic-addon2"><i class="fas fa-calendar-alt"></i></span></div></div></div>';
			} else {
				modal += '<div class="row"><div class="col-sm"><label>' + header + '</label></div><div class="col-sm"><input class="form-control" type="text" name="' + header + '" /></div>';
			}
		} else {
			if (header.toLowerCase().includes('date') || header.toLowerCase().includes('lastconnected')) {
				modal += '<div class="col-sm"><label>' + header + '</label></div><div class="col-sm"><div id="date" class="input-group"><input type="text" ' + disabled + ' class="form-control datepicker_recurring_start" name="' + header + '" /><div class="input-group-append"><span class="input-group-text" id="basic-addon2"><i class="fas fa-calendar-alt"></i></span></div></div></div></div><br>';
			} else {
				modal += '<div class="col-sm"><label>' + header + '</label></div><div class="col-sm"><input class="form-control" type="text" name="' + header + '" /></div></div><br>';
			}
		}
		oddCheck++;
	}
	...
	// Add delete modal (even though it won't be used for empty tables)
	modal += '<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Delete Record</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><div class="modal-body"><p>Are you sure you want to delete this record?<br>This cannot be undone.</p></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button><button type="button" class="btn btn-danger" data-dismiss="modal" onclick="deleteRecord(\'' + tablename + '\');">Delete</button></div></div></div></div>';
