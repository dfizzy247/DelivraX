...
@rest_api.route("/")
def apiDefault():
    return make_response(jsonify([{"Name":"DelivraX REST API","Version":"0.1","Created":"08/02/2020","LastModified":"29/02/2020"}]))
...
def returnDrivers():
  if request.method == 'GET':
    ...
    try:
     ...
     cur = conn.cursor()
     cur.execute("SELECT * FROM Drivers LIMIT 0")
     # Get column headers from table structure (works even if table is empty)
     params = []
     headers = []
     row_headers=[x[0] for x in cur.description]
     ...
     #Generate driver username (only if FirstName and LastName are provided)
     if 'Username' in request.args and request.args['Username'] != 'None':
         # Username already provided, use it
         uname = _json['Username']
     elif 'FirstName' in request.args and 'LastName' in request.args:
         ...
         # No username or names provided, generate a random one
         import random
         uname = 'driver' + str(random.randint(1000, 9999))
...
@rest_api.route('/drivers/login', methods=['GET', 'POST'])
def driver_login():
    _json = request.args
    
    # Check if required parameters are provided
    if 'Username' not in _json or 'Password' not in _json:
        return make_response(jsonify([{"Status":"Error","Message":"Username and Password are required"}]), 400)
...
def getMoney():
    try:
        ...
        for month in range(1, current_month):
            ...
            if amount != None and amount != "No data for this month":
               json += '"Amount":"'+str(round(amount,2))+'",'
            else:
               json += '"Amount":0,'
            if receipt != None and receipt != "No data for this month":
...
def monthRevenue(month):
    try:
        # Check if PricePaid column exists, otherwise return None
        ...
        # Column doesn't exist or other error - return 0
        return 0
